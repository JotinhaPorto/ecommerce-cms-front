...
import { deleteSize } from "@/api/store";
...
const CellAction = ({ data }: CellActionProps) => {
  ...
  const handleDelete = async (id: string) => {
    try {
      await deleteSize(params.storeId as string, id);
      toast({
        className: cn(
          "top-0 right-0 flex fixed md:max-w-[420px] md:top-4 md:right-4"
        ),
        title: "Tamanho deletado!",
        description: "O tamanho foi deletado com sucesso.",
      });
      router.refresh();
    } catch (error) {
      console.log(error);
  ...
  const handleUpdate = (id: string) => {
    router.push(`/${params.storeId}/tamanhos/${id}`);
  };
